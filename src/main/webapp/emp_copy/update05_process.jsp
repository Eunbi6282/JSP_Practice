...
<body>
	...
	<%
		...
		try{
			...
			sql = "select * from emp_copy where eno = ?";  // 사원번호같으면 update
			...
			rs = pstmt.executeQuery();
				...
				//pstmt.executeQuery() : select, Resultset 객체에 값 넣기
			...
			if(rs.next()){	//true => 사원이름 비교
				...
				if(name.equals(rname)){
					...
					// 폼에서 넘겨받은 값들로 update
				sql = "update emp_copy set manager = ? , commission = ? where eno = ?";
				pstmt = conn.prepareStatement(sql); 	// PreparedStatement -> sql 동시에 선언
				pstmt.setInt(1, manager);
				pstmt.setInt(2, commission);
				pstmt.setInt(3,eno);
